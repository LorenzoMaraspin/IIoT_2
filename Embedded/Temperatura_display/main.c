...
void main(void) {
    ...
    //setto la linea 1 del display LCD per visualizzare i vari dati
    Send_Command(L_L1);
    while(1)
    {
        //acquisisco il valore del sensore di temperatura
        valore = readADC(2);
        
        //se il valore è variato da quello vecchio entra nell if
        if(vecchio != valore){
            //pulisco lo schermo
            Send_Command(L_CLR);
            
            //calcoli per convertire il valore raccolto in gradi centigradi
            //
            volt = valore * 4.88281;
            temp = volt / 10;
            temp1 = temp * 100;
            
            //calcoli per visualizzare i vari valori sul display LCD
            m = (temp1 / 1000) + '0';
            c = ((temp1 % 1000) / 100) + '0';
            d = (((temp1 % 1000)%100) /10) +'0';
            u = ((temp1 % 10)+'0');
            
            //scrivo a video LCD i valori della temperatura
            send_char(m);
            send_char(c);
            send_char(d);
            send_char(u);
...
// funzione che manda i vari caratteri da visualizzare sullo schermo LCD
